...
func (ws *WireService) Start() {
	...
	log.Infof("Starting wire service at height %d %s", int(best.Height), best.Header.BlockHash().String())
out:
	for {
		select {
		case m := <-ws.msgChan:
...
func (ws *WireService) startSync(syncPeer *peerpkg.Peer) {
	...
	if bestPeer != nil {
		...
		// all we care is headers
		log.Infof("Starting chain download from %s", bestPeer)
		err = bestPeer.PushGetHeadersMsg(locator, &ws.zeroHash)
		if err != nil {
...
func (ws *WireService) handleHeadersMsg(hmsg *headersMsg) {
	...
	if badHeaders > 1 {
		log.Warnf("Disconnecting from peer %s because he sent us too many bad headers", peer.String())
		peer.Disconnect()
		return
	}
...
//	} else {
//		log.Debugf("Ingested new tx %s at height %d from %s", txHash.String(), ht.height, peer.Addr())
//	}
//
