...
func TestUtxoPut(t *testing.T) {
	...
	stmt, _ := uxdb.db.Prepare("select outpoint, value, height, scriptPubKey, lock from utxos where outpoint=?")
	defer stmt.Close()
	...
	err = stmt.QueryRow(o).Scan(&outpoint, &value, &height, &scriptPubkey, &lockInt)
	if err != nil {
		t.Error(err)
	}
	if outpoint != o {
	...
	if lockInt != 0 {
		t.Error("Utxo db returned wrong lock")
	}
}
...
func TestLock(t *testing.T) {
	...
	stmt, _ := uxdb.db.Prepare("select lock from utxos where outpoint=?")
	defer stmt.Close()

	var lockInt int
	o := utxo.Op.Hash.String() + ":" + strconv.Itoa(int(utxo.Op.Index))
	err = stmt.QueryRow(o).Scan(&lockInt)
	if err != nil {
		t.Error(err)
	}
	if lockInt != 1 {
		t.Error("Utxo db returned wrong lock value")
	}
}
