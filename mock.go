...
func (m *mockUtxoStore) Get(addr string) *wallet.Utxo {
	return m.utxos[addr]
}
...
func (m *mockUtxoStore) Lock(utxo wallet.Utxo) error {
	key := utxo.Op.Hash.String() + ":" + strconv.Itoa(int(utxo.Op.Index))
	u, ok := m.utxos[key]
	if !ok {
		return errors.New("Not found")
...
func (m *mockUtxoStore) Unlock(op wire.OutPoint) error {
	key := op.Hash.String() + ":" + strconv.Itoa(int(op.Index))
	u, ok := m.utxos[key]
	if !ok {
		return errors.New("Not found")
	}
